    ...
    (set, get) => ({
      ...
      login: async (email: string, password: string) => {
        ...
        try {
          ...
          if (!response.ok) {
            console.error('[Auth] Login failed:', { status: response.status, statusText: response.statusText });
            set({ isLoading: false });
            return false;
          }
          ...
          try {
            ...
            if (tokenParts.length === 3) {
              ...
              console.log('[Auth] Login complete, user:', { id: user.id, email: user.email, role: user.role });
              return true;
            }
          } catch (decodeErr) {
          ...
          console.error('[Auth] Login error:', err instanceof Error ? { message: err.message, stack: err.stack } : err);
          set({ isLoading: false });
          return false;
        }
      },
