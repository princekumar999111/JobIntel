...
exports.updateAutoScrapeSchedule = updateAutoScrapeSchedule;
exports.updateCompanyFilters = updateCompanyFilters;
exports.updateDataQualityFilters = updateDataQualityFilters;
exports.getScraperCostSummary = getScraperCostSummary;
exports.testScraperConfig = testScraperConfig;
const ScraperConfig_1 = require("../models/ScraperConfig");
 ...
 */
async function getScraperConfig(req, res) {
    try {
        let config = await ScraperConfig_1.ScraperConfig.findOne().select('-__v');
        // If no config exists, create default one
        if (!config) {
            config = await ScraperConfig_1.ScraperConfig.create({
                enabled: true,
 ...
 */
async function updateScraperConfig(req, res) {
    try {
        ...
        const oldConfig = await ScraperConfig_1.ScraperConfig.findOne();
        if (!oldConfig) {
        ...
        const updated = await ScraperConfig_1.ScraperConfig.findOneAndUpdate({}, {
            ...updates,
 ...
 */
async function updateBudgetSettings(req, res) {
    try {
 ...
 */
async function updateAutoScrapeSchedule(req, res) {
    try {
 ...
 */
async function updateCompanyFilters(req, res) {
    try {
 ...
 */
async function updateDataQualityFilters(req, res) {
    try {
 ...
 */
async function getScraperCostSummary(req, res) {
    try {
        const config = await ScraperConfig_1.ScraperConfig.findOne();
        if (!config) {
            return res.status(404).json({
                success: false,
                error: 'Scraper config not found',
 ...
 */
function calculateNextRun(timeStr, frequency) {
    const now = new Date();
 ...
 */
async function testScraperConfig(req, res) {
    try {
        const config = await ScraperConfig_1.ScraperConfig.findOne();
        if (!config) {
            return res.status(404).json({
