...
const razor = new razorpay_1.default({ key_id: key_id || '', key_secret: key_secret || '' });
const PLANS = {
...
async function createOrder(req, res) {
    try {
        ...
        const payment = await Payment_1.Payment.create({ userId: user._id, amount, currency, razorpayOrderId: order.id, status: 'created' });
...
async function verifyPayment(req, res) {
    try {
        ...
        const user = req.user;
        const payment = await Payment_1.Payment.findOne({ razorpayOrderId: razorpay_order_id });
        if (!payment)
        ...
        if (payment.status === 'paid')
            return res.json({ ok: true, message: 'Already verified', expiresAt: await Subscription_1.Subscription.findOne({ userId: payment.userId, active: true }).then(s => s?.expiresAt) });
        ...
        const existing = await Subscription_1.Subscription.findOne({ userId: payment.userId, active: true });
        if (!existing) {
            await Subscription_1.Subscription.create({ userId: payment.userId, plan: 'premium', startDate: start, expiresAt: expires, active: true });
        }
        // upgrade user tier
        await User_1.User.findByIdAndUpdate(payment.userId, { tier: 'premium' });
...
async function razorpayWebhook(req, res) {
    try {
        ...
        const expected = crypto_1.default.createHmac('sha256', webhookSecret).update(body).digest('hex');
        ...
        if (evType === 'payment.captured') {
            ...
            let payment = null;
            if (orderId)
                payment = await Payment_1.Payment.findOne({ razorpayOrderId: orderId });
            if (!payment && paymentId)
                payment = await Payment_1.Payment.findOne({ razorpayPaymentId: paymentId });
            if (payment) {
