...
class JobMatcher {
     ...
     */
    static async findMatches(user, limit = 10) {
        try {
            // Get admin-posted jobs
            const adminJobs = await Job_1.Job.find({ status: 'active' }).lean().limit(limit * 2);
            // Get scraped jobs (most recent)
            const scrapedJobs = await ScraperJob_1.ScraperJob.find({
                status: 'active',
                archived: false
            })
                .lean()
     ...
     */
    static calculateMatch(user, job, source = 'admin') {
        let matchScore = 0;
        const matchedSkills = [];
        const missingSkills = [];
        const reasons = [];
     ...
     */
    static async getMatchStats(user) {
        try {
            const allJobs = await Job_1.Job.find({ status: 'active' }).lean();
            let highMatches = 0;
            let mediumMatches = 0;
            let lowMatches = 0;
