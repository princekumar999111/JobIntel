...
async function callOpenAISystemPrompt(system, userPrompt) {
    const payload = {
    ...
    const res = await (0, node_fetch_1.default)(OPENAI_URL, {
        method: 'POST',
        headers: {
...
function simpleHeuristicParse(htmlOrText) {
    // Improved heuristic parser with JSON-LD and microdata helpers
    const out = { title: null, company: null, location: null, salary: null, description: null };
    const html = htmlOrText || '';
    // Helper: try extract JSON-LD JobPosting
    const extractJsonLd = (h) => {
        try {
                ...
                try {
                    ...
                    for (const it of items) {
                        ...
                        for (const k of Object.keys(it)) {
                            const v = it[k];
                            if (v && typeof v === 'object' && (v['@type'] === 'JobPosting' || (Array.isArray(v['@type']) && v['@type'].includes('JobPosting'))))
                                return v;
                        }
                    }
                }
    ...
    // location heuristics
    if (/Remote|Work from home|Work from anywhere|WFH/i.test(text))
        out.location = 'Remote';
    else {
    ...
    if (salaryM)
        out.salary = salaryM[0];
    out.description = lines.slice(0, 6).join('\n');
    out.confidence = 0.5;
    return out;
}
async function parseJob(raw, useCache = true) {
    const key = `parse:${(raw.html || raw.text || '').slice(0, 1000)}`;
    if (useCache) {
        const c = (0, aiCache_1.getCache)(key);
        if (c)
            return c;
    }
    ...
    if (!OPENAI_KEY) {
        ...
        (0, aiCache_1.setCache)(key, res, 60 * 5);
        return res;
    }
    try {
        ...
        (0, aiCache_1.setCache)(key, parsed, 60 * 60);
        return parsed;
    }
    catch (e) {
        const fallback = simpleHeuristicParse(content);
        (0, aiCache_1.setCache)(key, fallback, 60 * 5);
        return fallback;
    }
}
async function matchCandidate(jobFields, candidateProfile) {
    const key = `match:${JSON.stringify(jobFields).slice(0, 500)}:${JSON.stringify(candidateProfile).slice(0, 500)}`;
    const cached = (0, aiCache_1.getCache)(key);
    if (cached)
        return cached;
    if (!OPENAI_KEY) {
...
async function generateCoverLetter(jobFields, candidateProfile) {
    ...
    if (!OPENAI_KEY) {
        ...
        (0, aiCache_1.setCache)(key, out, 60 * 60);
        return out;
    }
    try {
...
exports.default = { parseJob, matchCandidate, generateCoverLetter };
