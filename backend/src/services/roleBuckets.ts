...
export interface IRoleBucket {
  ...
  careerLevel: string[];
  domain: string[];
  priority: number; // 1 = highest (freshers first)
  recommendedFrequency: 'daily' | 'twice-weekly' | 'weekly'; // How often to scrape
  searchStrategy: 'exact' | 'phrase' | 'combined';
}

export const ROLE_BUCKETS: IRoleBucket[] = [
  // ================================================
...
export const getBucketById = (bucketId: string): IRoleBucket | undefined => {
  return ROLE_BUCKETS.find(b => b.id === bucketId);
};

export const getBucketsByPriority = (): IRoleBucket[] => {
  return [...ROLE_BUCKETS].sort((a, b) => a.priority - b.priority);
};

export const getFresherPriority = (): IRoleBucket[] => {
  // Return top priority buckets for fresher-first approach
  return ROLE_BUCKETS.filter(b => b.priority <= 4).sort((a, b) => a.priority - b.priority);
};

export const getBucketsByDomain = (domain: string): IRoleBucket[] => {
  return ROLE_BUCKETS.filter(b => b.domain.includes(domain))
    .sort((a, b) => a.priority - b.priority);
};

export const getSearchKeywordsCombined = (): string[] => {
  // Get all keywords across all buckets for bulk searches
  return [...new Set(ROLE_BUCKETS.flatMap(b => b.keywords))];
};

export const getPrimaryBuckets = (): IRoleBucket[] => {
  // Get primary (non-utility) buckets
  return ROLE_BUCKETS.filter(b => !['experience-level', 'employment-type', 'work-mode'].includes(b.id))
    .sort((a, b) => a.priority - b.priority);
};
...
export const BUCKET_KEYWORDS_SUMMARY = {
