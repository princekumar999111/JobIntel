...
import { Company, ICompany, CompanyStatus, VerificationStatus } from "../models/Company";
...
export async function listCompanies(req: Request, res: Response) {
  try {
    ...
    if (blacklisted === 'true') filter.blacklistedFromScraping = true;
    if (priority) filter.scraperPriority = priority;
...
export async function updateCompany(req: Request, res: Response) {
  try {
    ...
    company.lastUpdatedBy = (req as any).user?.id;
    await company.save();
...
export async function verifyCompany(req: Request, res: Response) {
  try {
    ...
    if (approved) {
      ...
      company.verifiedBy = (req as any).user?.id;
...
export async function bulkImportCompanies(req: Request, res: Response) {
  try {
...
export async function getCompanyStatistics(req: Request, res: Response) {
  try {
    const [
      totalCompanies,
      verifiedCompanies,
      ...
      Company.countDocuments({ blacklistedFromScraping: true }),
      Company.countDocuments({ scraperPriority: 'high' }),
      Company.aggregate([
...
export async function updateScrapingMetrics(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const {
      jobsPosted,
    ...
    if (jobsPosted !== undefined) company.jobsPosted = jobsPosted;
    if (jobsScraped !== undefined) company.jobsScraped = jobsScraped;
    if (dataQualityScore !== undefined) company.dataQualityScore = Math.max(0, Math.min(100, dataQualityScore));
    if (averageSalaryDataQuality !== undefined) company.averageSalaryDataQuality = Math.max(0, Math.min(100, averageSalaryDataQuality));
    if (averageDescriptionLength !== undefined) company.averageDescriptionLength = averageDescriptionLength;
    if (activeJobs !== undefined) company.activeJobs = activeJobs;
    if (totalApplications !== undefined) company.totalApplications = totalApplications;
